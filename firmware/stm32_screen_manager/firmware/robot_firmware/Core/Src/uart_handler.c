...
volatile bool uartStartMatchFlag = false;
...
void UART_Process_Decoded_Message(uint16_t function, uint16_t payloadLength, const uint8_t *payload)
{
    ...
    case UART_CMD_START_MATCH: // 0x0006
        uartStartMatchFlag = true;
        break;
    ...
    case UART_CMD_POINTS:
        if (payloadLength == 4U)
        {
            uint32_t pts = ((uint32_t)payload[0] << 24) |
            ...
            // Log via UART_Send_Raw
            {
                ...
                int len = snprintf(buf, sizeof(buf),
                                   "UART: CMD_POINTS reçu, pts=%lu\r\n",
                                   (unsigned long)pts);
                if (len > 0)
                    UART_Send_Raw(buf);
            }

            // Envoi vers la queue FreeRTOS
            osMessageQueuePut(pointsQueueHandle, &pts, 0U, 0U);
        }
        break;
