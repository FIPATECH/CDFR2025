...
void uart_send_start() {
  UART_Encode_And_Send_Message_Port(UART_PORT_3, UART_CMD_START_MATCH, 0, nullptr);
}
void uart_send_stop() {
  UART_Encode_And_Send_Message_Port(UART_PORT_3, UART_CMD_STOP_MATCH, 0, nullptr);
}
void uart_send_pong() {
  UART_Encode_And_Send_Message_Port(UART_PORT_3, UART_CMD_PONG, 0, nullptr);
}
...
// Ajoutez en fin de uart_handler.cpp, après uart_task(...)
void uart_poll() {
  // — réception (décode les octets entrants) —
  while (Serial2.available()) decode_byte(dec2, Serial2.read(), UART_PORT_2);
  while (Serial3.available()) decode_byte(dec3, Serial3.read(), UART_PORT_3);

  // — transmission (vide les buffers TX) —
  if (isTx2 && Serial2.availableForWrite()) {
    if (circular_buffer_pop(&txCB2, &tx_byte2)) Serial2.write(&tx_byte2, 1);
    else isTx2 = false;
  }
